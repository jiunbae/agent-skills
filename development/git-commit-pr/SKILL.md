...
**토큰 절약 효과:**
```
Before: 6-8회 도구 호출 (git status, diff, grep 등)
After:  1회 스크립트 호출
절약률: 70-80%
```
...
```
┌─────────────────────────────────────────────────────────┐
│ 1. commit-check.sh collect (1회 호출)                   │
│    ├─ 변경사항 수집 (staged, unstaged, untracked)       │
│    ├─ 최근 커밋 스타일 확인                              │
│    └─ 5단계 보안 검증 수행                               │
├─────────────────────────────────────────────────────────┤
│ 2. LLM 판단                                             │
│    └─ 커밋 메시지 작성 (프로필 스타일 적용)              │
├─────────────────────────────────────────────────────────┤
│ 3. 커밋 실행                                             │
│    └─ git add → git commit                              │
└─────────────────────────────────────────────────────────┘
...
| 파일 | 설명 |
|------|------|
| `scripts/commit-check.sh` | 커밋 전 보안 검증 및 변경사항 수집 스크립트 |
