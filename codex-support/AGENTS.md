...
- 작업 시작 전에, 현재 세션에서 1회 `agt skill list --json`을 실행해 `skills[]` 목록을 확보합니다.
...
- 스킬이 Claude Code 전용 오케스트레이션에 가깝다고 판단되면, 필요 시 `agt run --skill <name> "..."`로 Claude에게 위임할 수 있습니다(단, 결과 검토/반영은 Codex가 수행).
