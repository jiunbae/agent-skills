...
- `**활성화**: false`이면 `--check`를 제외한 조회·생성·수정·초기화 명령을 모두 차단합니다.
- 작업 ID는 단일 `task-*` 식별자, 프로젝트명은 단일 디렉토리명만 허용합니다. 절대 경로, `..` 경로 이동, 경로 구분자가 포함된 값은 거부합니다.
- Vault 밖을 가리키는 심볼릭 링크는 읽기와 쓰기 모두 거부하며, 명령 출력에는 설정된 절대 Vault 경로를 표시하지 않습니다.
...
# 경로를 노출하지 않는 설정/활성화 상태 확인
./scripts/obsidian-tasks.py --check
```
